...
/**
 * This is a cache for caching raw channel elements
 *
 ...
 * 2) the history changed and we want to update only messages before the last loaded element -> time parameter
        ...
        serializeWithType('RawChannelEntriesCache', async () => {
            ...
            for await (const elem of ChannelManager.mergeIteratorMostCurrent([
                ...
                if (elem.activeIteratorCount === 2) {
                    // Phase 1: as long as both iterators are active we pick all elements from the new iterator (it also
