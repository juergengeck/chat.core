...
export interface TrustPlan {
  setTrustLevel(request: { personId: SHA256IdHash<Person>; trustLevel: string; reason?: string }): Promise<void>;
}
...
export class TopicGroupManager {
  ...
  async createGroupTopic(
    ...
    const topic = await this.oneCore.topicModel.createGroupTopic(
      topicName,
      allParticipants,
      topicId,
    ...
    const certResult = await this.oneCore.leuteModel.trust.certify(
      ...
      { data: topic.channel as unknown as SHA256Hash },
      this.oneCore.ownerId
    );
   ...
   */
  async getTopicParticipants(topicId: string): Promise<SHA256IdHash<Person>[]> {
    ...
    const personSet: Set<SHA256IdHash<Person>> = hashGroup.person || new Set();
   ...
   */
  async addParticipantsToTopic(topicId: string, newParticipants: SHA256IdHash<Person>[]): Promise<void> {
    ...
    const certResult = await this.oneCore.leuteModel.trust.certify(
      'AffirmationCertificate',
      { data: savedChannelInfo.idHash as unknown as SHA256Hash },
      this.oneCore.ownerId
    );
  ...
  async createP2PTopic(
    topicName: string,
    topicId: string,
    participants: [SHA256IdHash<Person>, SHA256IdHash<Person>]
  ): Promise<Topic> {
    ...
    const topic = await this.oneCore.topicModel.createOneToOneTopic(
      participants[0],
      participants[1]
    );
   ...
   */
  async ensureP2PChannelsForPeer(peerPersonId: SHA256IdHash<Person>): Promise<void> {
    ...
    if (!topic) {
      topic = await this.oneCore.topicModel.createOneToOneTopic(
        this.oneCore.ownerId,
        peerPersonId
      );
   ...
   */
  async handleReceivedTopic(topicIdHash: SHA256IdHash<Topic>, topic: Topic): Promise<boolean> {
    ...
    if (topic.channelCertificate) {
      try {
        ...
        const knownPersonIds = await Promise.all(
          knownPeople.map(someone => someone.mainIdentity())
        );
        ...
        console.warn(`[TopicGroupManager] Could not validate certificate:`, (error as Error).message);
    ...
    import('@refinio/one.core/lib/storage-versioned-objects.js').then(({ onVersionedObj }) => {
      onVersionedObj.addListener(async (result: VersionedObjectResult) => {
        if (result.obj.$type$ === 'Topic') {
          await this.handleReceivedTopic(result.idHash as SHA256IdHash<Topic>, result.obj as Topic);
        }
      });
  ...
  async getOrCreateConversationGroup(topicId: string): Promise<unknown> {
