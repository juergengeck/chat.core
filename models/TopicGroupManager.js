...
export class TopicGroupManager {
     ...
     * @param topic - Optional: pass the Topic object directly to avoid re-fetching from registry
     */
    async getTopicParticipants(topicId, topic) {
        console.log(`[TopicGroupManager] Getting participants for topic: ${topicId}`);
        // Use passed topic or fetch from registry
        const resolvedTopic = topic ?? await this.oneCore.topicModel.findTopic(topicId);
        if (!resolvedTopic) {
            throw new Error(`Topic ${topicId} not found`);
        }
        // Get ChannelInfo
        const channelInfoResult = await this.storageDeps.getObjectByIdHash(resolvedTopic.channel);
        const channelInfo = channelInfoResult.obj;
        // Get HashGroup
    ...
    async handleReceivedTopic(topicIdHash, topic) {
        ...
        if (this.oneCore.paranoiaLevel === 0 && this.trustPlan) {
            // Pass topic directly to avoid re-fetching from registry
            const participants = await this.getTopicParticipants(topic.id, topic);
     ...
     *
     * Uses TopicModel.onTopicReady instead of raw onVersionedObj to ensure
     * the topic is in the registry before we try to access it (fixes race condition).
     *
     * @param topicModel - TopicModel instance (required). Pass from deps, not oneCore,
     *                     to ensure it's available during module initialization.
     */
    initializeTopicSyncListener(topicModel) {
        console.log(`[TopicGroupManager] Initializing topic sync listener via TopicModel.onTopicReady`);
        if (!topicModel) {
            throw new Error('[TopicGroupManager] topicModel is required for initializeTopicSyncListener');
        }
        // Listen to TopicModel's onTopicReady - fires AFTER topic is in registry
        // OEvent is a functor - call it directly to subscribe (returns unsubscribe fn)
        topicModel.onTopicReady(async (topic, idHash) => {
            await this.handleReceivedTopic(idHash, topic);
        });
