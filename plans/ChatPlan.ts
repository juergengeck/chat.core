...
export class ChatPlan {
  ...
  async getConversations(request: GetConversationsRequest): Promise<GetConversationsResponse> {
    try {
      ...
      const contactMap = new Map<string, {name: string; isAI: boolean; modelId?: string}>();
      ...
      if (this.nodeOneCore.leuteModel) {
        try {
          const others: any = await this.nodeOneCore.leuteModel.others();
          await Promise.all(others.map(async (someone: any) => {
            try {
              const personId: any = await someone.mainIdentity();
      ...
      const conversations = await Promise.all(
        topicsWithHashes.map(async ({topic, idHash}: {topic: any; idHash: string}) => {
          ...
          let isAITopic = false;
          let aiResponding: string | undefined;
